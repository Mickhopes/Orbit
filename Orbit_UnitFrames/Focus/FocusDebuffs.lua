...
local SYSTEM_ID = "Orbit_FocusDebuffs"
local SYSTEM_INDEX = 1

local Constants = Orbit.Constants

local Plugin = Orbit:RegisterPlugin("Focus Debuffs", SYSTEM_ID, {
    defaults = {
        IconsPerRow = 5,
        MaxRows = 2,
        Spacing = 2,
...
function Plugin:IsEnabled()
    ...
    if focusPlugin and focusPlugin.GetSetting then
        local enabled = focusPlugin:GetSetting(FOCUS_FRAME_INDEX, "EnableDebuffs")
        if enabled ~= nil then
            return enabled
        end
    end
...
function Plugin:OnLoad()
    ...
    Frame.editModeName = "Focus Debuffs"
    ...
    if focusFrame then
        ...
        Frame:SetPoint("TOPLEFT", focusFrame, "BOTTOMLEFT", 0, -50)
        OrbitEngine.Frame:CreateAnchor(Frame, focusFrame, "BOTTOM", 50, nil, "LEFT")
    else
...
function Plugin:UpdateDebuffs()
    ...
    for i, aura in ipairs(debuffs) do
        ...
        self:SetupAuraTooltip(icon, aura, "focus", "HARMFUL|PLAYER")
